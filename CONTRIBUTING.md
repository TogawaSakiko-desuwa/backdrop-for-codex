...
- Codex 精确版本号不得单独作为安全准入门槛或拒绝理由。安全身份/进程/会话/监听器/端点/目标验证必须先行且失败关闭；只有通过安全层的官方版本才能进入精确受审、显式受审版本带或通用结构探针。版本带必须有明确的包含下界和排除上界，不得无界扩张。
- 五项能力保持独立：全局背景、功能区域识别、玻璃样式、音频和高级内容表面。1.3.0 不得误报区域识别或音频已实现；一次 generation 内能力只允许降级。
- 初始注入最多等待 10 秒且只接受一个合格工作页；持续多目标歧义必须拒绝并清理，不能任意挑选或同时注入。
- schema 2 更改必须保持严格未知字段/引用/范围校验、同目录原子保存、V1 原始字节只读备份、恢复状态和未来 schema 只读语义；不得用默认设置静默覆盖异常文档。
- 不读取聊天，不修改/重签 Codex 包，不要求管理员权限，不静默添加自动启动。
- 不加入遥测、崩溃上传、更新检查或项目自有远程服务，除非治理文档、隐私说明、威胁模型和明确用户同意机制已先行评审。
- 日志不得包含聊天或媒体绝对路径；异常对象和 DTO 同样需要脱敏。诊断导出必须由用户主动触发、采用类型化固定白名单且不自动上传；禁止加入路径、文件名、页面标题/URL/DOM、聊天、设置、标识符、散列或原始异常文本。
...
- 精确探针、显式受审版本带与仅声明全局背景的通用探针选择及边界、五项能力独立降级、同 generation 不重新启用；
