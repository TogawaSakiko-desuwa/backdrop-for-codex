...
- 修复精密工作台的显式按钮与列表项样式截断 WPF-UI 动态前景色链、导致深色主题出现黑字的问题；设置对话框现在由全窗口模态层拦截输入，不再把后台工作台强制切入浅灰 Disabled 模板。
...
- Release 构建为 0 警告、0 错误；665 项非集成、非 browser-contract 测试全部通过，格式与差异检查无错误。
