...
兼容能力按实际页面结构判断，而不是只看 Codex 版本号。官方包、进程、当前会话、回环端点和唯一页面的安全核验始终先行且严格失败关闭；通过安全核验后，全局背景使用独立的最小结构基线。玻璃与高级表面只审查没有 `main` 祖先的顶层主内容候选，并综合候选自身的 typed shell-main，以及直接归属该候选的受审 header 和受审 viewport 三族信号。任意两族成立且只有一个顶层合格候选时才启用；嵌套路由主区域不能借出信号，同族重复不会增加可信度，多候选、单一信号或跨主区域散落都会保守降级。

这种策略可以容忍一族标记随小版本漂移，但不能保证任意上游 DOM 重写都无需适配。Glass 不可用而全局背景仍可用时，应用会使用自己拥有的高可读性对比回退，继续关闭不可用的可选效果，避免深色或浅色界面直接落在壁纸上而难以辨认；安全核验失败时仍不会继续注入。
