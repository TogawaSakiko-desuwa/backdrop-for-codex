...
- Version 1.5.0 supports local PNG, JPEG, WebP, MP4, and WebM files, plus installed Wallpaper Engine Image, Video, Scene, and Web projects from Workshop and Local Projects.
...
- Supported Wallpaper Engine content types are Image, Video, Scene, and Web. Application and unknown project types are not launched.
- Image and Video use validated local media. Scene and Web send only captured pixels into Codex; project HTML, JavaScript, executables, input, and audio are not forwarded.
- Scene and Web require Wallpaper Engine to be running. Their audio is left to Wallpaper Engine and may be audible; Backdrop transports pixels only.
